...
```bash
# Get products
curl http://localhost:8400/api/v1/products

# Create order
curl -X POST http://localhost:8400/api/v1/orders \
  -H "Content-Type: application/json" \
  -d '{"customer_id":123,"items":[...]}'

# Admin endpoint
curl http://localhost:8400/api/v1/admin/products \
  -H "Authorization: Bearer {token}"

# Drop shipper endpoint
curl http://localhost:8400/api/v1/dropship/orders \
  -H "X-API-Key: {api_key}"
```
