...
- MySQL/MariaDB
- XAMPP (recommended)
- maximus-backendadmin-api running on port 8400
...
5. Start server:
```bash
php artisan serve --port=8401
```

## API Connection

The admin site connects to the API at `http://localhost:8400/api/v1`.

To change, update the `API_BASE` constant in blade templates.

## First-Time Setup

1. Ensure API is running: `http://localhost:8400/api/v1`
2. Access admin: `http://localhost:8401/adminpanel`
3. Default login: Check users table in database

## Troubleshooting

### API Connection Failed
- Verify API server is running on port 8400
