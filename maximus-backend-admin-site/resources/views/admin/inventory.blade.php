...
<script>
    ...
    document.getElementById('confirmAdjust').addEventListener('click', async function() {
        ...
        try {
            ...
            const searchResponse = await fetch(`{{ rtrim(env("API_PUBLIC_URL", "http://localhost:8400/api/v1"), "/") }}/admin/inventory/products?search=${encodeURIComponent(productId)}&per_page=1`);
            ...
            const response = await fetch('{{ rtrim(env("API_PUBLIC_URL", "http://localhost:8400/api/v1"), "/") }}/admin/inventory/adjust-stock', {
                method: 'POST',
                headers: {
