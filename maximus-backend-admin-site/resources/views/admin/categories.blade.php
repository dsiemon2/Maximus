    ...
    tbody.innerHTML = categories.map((cat, index) => {
        ...
        return `
            <tr data-code="${cat.CategoryCode}" onclick="highlightRow(event)" style="cursor: pointer;">
                ...
                <td>
                    ${cat.image ?
                        `<img src="${cat.image.startsWith('http://') || cat.image.startsWith('https://') ? cat.image : STOREFRONT_URL + '/assets/' + cat.image}"
                             alt="${cat.Category}"
                             class="rounded"
...
function editCategory(code) {
    ...
    if (cat) {
        ...
        if (cat.image) {
            const imgSrc = cat.image.startsWith('http://') || cat.image.startsWith('https://') ? cat.image : `${STOREFRONT_URL}/assets/${cat.image}`;
            document.getElementById('editCurrentImg').src = imgSrc;
