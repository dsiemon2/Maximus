    ...
    <div class="card-body">
        <form class="row g-3" method="GET">
            ...
            <div class="col-md-3 text-end">
                ...
                <a href="{{ env('API_PUBLIC_URL', 'http://localhost:8400/api/v1') }}/admin/export/products" class="btn btn-outline-secondary" target="_blank"><i class="bi bi-download"></i> Export</a>
            </div>
        </form>
    </div>
</div>
                    ...
                    @php
                        ...
                        if ($imagePath) {
                            ...
                            if (str_starts_with($imagePath, 'http://') || str_starts_with($imagePath, 'https://')) {
                                ...
                                $storefrontUrl = rtrim(env('STOREFRONT_URL', 'http://localhost:8400'), '/');
...
const STOREFRONT_URL = '{{ rtrim(env("STOREFRONT_URL", "http://localhost:8400"), "/") }}';
let allSuppliers = [];
...
function renderEditImagesGallery() {
    ...
    editProductImages.forEach((img, index) => {
        ...
        // Use storage path or legacy Image field - check for full URLs first
        let imgSrc = '';
        if (img.image_path) {
            if (img.image_path.startsWith('http://') || img.image_path.startsWith('https://')) {
                imgSrc = img.image_path;
            } else if (img.image_path.startsWith('products/')) {
                imgSrc = `${STOREFRONT_URL}/storage/${img.image_path}`;
            } else {
                imgSrc = `${STOREFRONT_URL}/assets/${img.image_path}`;
            }
        }
    ...
    try {
        ...
        if (result.success && result.data && result.data.length > 0) {
            ...
            if (productResult.success && productResult.data && productResult.data.Image) {
                const legacyImgUrl = getImageUrl({ image_path: productResult.data.Image });
                container.innerHTML = `
                    <img src="${legacyImgUrl}"
                         class="img-fluid rounded" alt="Product image"
