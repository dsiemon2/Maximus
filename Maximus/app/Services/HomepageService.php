...
class HomepageService
{
    ...
    {
        $settings = $this->getCategoryDisplaySettings();
        $ids = $settings['featured_category_ids'] ?? '100,200,300,400,500,600';
    ...
    protected function getDefaultCategoryDisplaySettings(): array
    {
        return [
            ...
            'featured_category_ids' => '100,200,300,400,500,600',
        ];
    }
...
<div id=\"{$carouselId}\" class=\"carousel {$slideClass} homepage-banner-carousel\" data-bs-ride=\"carousel\" data-bs-interval=\"{$slideDuration}\">";
        ...
        foreach ($banners as $index => $banner) {
            ...
            // Build image URL - use APP_URL for browser access
            $appUrl = rtrim(env('APP_URL', 'http://localhost:8400'), '/');
            $imageUrl = strpos($desktopImage, 'http') === 0 ? $desktopImage : "{$appUrl}/{$desktopImage}";
