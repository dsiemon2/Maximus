...
<div class="container my-4">
    ...
    @if($products->count() > 0)
            ...
            <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        @foreach($products as $product)
                            <th class="text-center" style="min-width: 200px;">
                                <form action="{{ route('products.compare.remove') }}" method="POST" class="d-inline">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $product->ID }}">
                                    <button type="submit" class="btn btn-sm btn-outline-danger float-end" title="Remove">
                                        <i class="bi bi-x"></i>
                                    </button>
                                </form>
                            </th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @foreach($products as $product)
                            <td class="text-center">
                                <a href="{{ route('cart.add', ['upc' => $product->UPC ?: $product->ItemNumber]) }}"
        ...
        <div class="d-flex justify-content-between mt-4">
            ...
            <form action="{{ route('products.compare.remove') }}" method="POST">
                @csrf
