...
@section('title', config('app.name') . ' - Premium Pet Food, Toys & Supplies')
...
@php
    ...
    if ($apiFeaturedCategories !== null && !empty($apiFeaturedCategories)) {
        ...
        // Default category descriptions (fallback) - Pet categories
        $categoryDescriptions = [
            100 => "Everything for your canine companion. Food, treats, toys, beds, and more.",
            200 => "Premium products for cats. Food, litter, toys, scratchers, and accessories.",
            300 => "Everything for pet birds. Seeds, cages, toys, and cage accessories.",
            400 => "Aquarium supplies and fish food for freshwater and saltwater fish.",
            500 => "For rabbits, hamsters, guinea pigs & more small animals.",
            600 => "Terrariums, heating, lighting, and food for reptiles.",
        ];
    }
...
<section class="hero-section">
    <div class="container text-center">
        <h1 class="display-3 fw-bold text-white mb-3">Welcome to {{ config('app.name') }}</h1>
        <p class="lead text-white mb-4">
            Premium pet food, toys, and supplies for your furry friends.<br>
            <strong>QUALITY. AFFORDABLE. AUTO-SHIP & SAVE 10%</strong>
        </p>
...
<section id="featured" class="container my-5">
    <div class="text-center mb-5">
        <h2 class="display-5 fw-bold mb-3" style="color: var(--theme-primary);">Shop by Pet</h2>
        <p class="lead text-muted">Explore our collection of premium pet supplies</p>
    </div>
    ...
    <div class="row g-4">
        @foreach($featuredCategories as $category)
            @php
                ...
                $description = $categoryDescriptions[$catCode] ?? 'Premium pet supplies for your beloved companion.';
                $catImage = is_array($category) ? ($category['image'] ?? '') : ($category->image ?? '');
                // Handle external URLs (Unsplash, etc.)
                if (!empty($catImage)) {
                    if (str_starts_with($catImage, 'http://') || str_starts_with($catImage, 'https://')) {
                        $imagePath = $catImage;
                    } else {
                        $imagePath = asset('assets/' . $catImage);
                    }
                } else {
...
<section id="featured-products" class="container my-5">
    <div class="text-center mb-5">
        <h2 class="display-5 fw-bold mb-3" style="color: var(--theme-primary);">{{ $featuredProductsTitle }}</h2>
    ...
    <div class="container">
        <div class="row g-4">
            <div class="col-12 col-md-4 text-center">
                <div class="info-card">
                    <i class="bi bi-truck display-4 mb-3" style="color: var(--theme-accent);"></i>
                    <h3>Fast Shipping</h3>
            ...
            <div class="col-12 col-md-4 text-center">
                <div class="info-card">
                    <i class="bi bi-shield-check display-4 mb-3" style="color: var(--theme-accent);"></i>
            ...
            <div class="col-12 col-md-4 text-center">
                <div class="info-card">
                    <i class="bi bi-headset display-4 mb-3" style="color: var(--theme-accent);"></i>
